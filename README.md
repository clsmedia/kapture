...
> The Test API is **opt-in**. Set `API_AUTH_REQUIRED=true` in `.env` to expose it; every request to `/api/v1/*` must then include `Authorization: Bearer <API_TOKEN>`.
...
| Concept | Purpose |
|---|---|
| `captureId` | Identifies **one** captured HTTP request. Generated by Kapture, returned by the webhook endpoint. |
...
| `API_TOKEN` | Authorizes access to the Test API (`Authorization: Bearer <API_TOKEN>`). Never used as a resource identifier. |
