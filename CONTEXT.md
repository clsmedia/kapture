...
Value object describing a capture query: captureId, correlationId, HttpMethod, uri substring, CapturedAt after/before bounds, a calendar-day filter (capturedOn), and optional limit/offset/order. Passed to `CapturedRequestRepository::findByCriteria()`, which both the Filesystem and SQLite repositories implement. Day granularity is adapter-native: the Filesystem adapter selects that day's archive file, SQLite matches its `captured_at_date` column.

## Capture Query
The single querying path for captured requests: criteria in, page out (`QueryCapturedRequests::page()`). Both the admin dashboard listing and the Test API listing go through it; pagination arithmetic (page → limit/offset) lives in `CapturedRequestCriteria::paginated()` / `::onDate()`. The dashboard variant additionally assembles archive navigation metadata (available dates, per-day counts) into the view model.
